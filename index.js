        ...
        login({ appState: JSON.parse(appState) }, (err, api) => {
            ...
            api.listenMqtt((err, message) => {
                ...
                if (message && message.body && allowedThreadIDs.includes(message.threadID)) {
                    if (message.body.toLowerCase().startsWith("fb ")) {
                        const content = message.body.slice(3).trim();
                        api.setMessageReaction("👍", message.messageID);
                        postToFacebook(api, content)
                            .then(info => {
                                api.sendMessage(ttt+"\n"+"โพสสำเร็จ!", message.threadID, message.messageID);
                            })
                            .catch(err => {
                                api.sendMessage(ttt+"\n"+"เกิดข้อผิดพลาดในการโพส: " + err.message, message.threadID, message.messageID);
                                api.setMessageReaction("👎", message.messageID);
                            });
                    } else if (isLink(message.body)) {
                    } else {
                        message.body = message.body.toLowerCase();
                        ...
                        if (message.body.toLowerCase().startsWith("#")) {
                            ...
                            fs.readFile("key.txt", "utf8", (err, data) => {
                                ...
                                if (randomMessageEnabled[message.threadID]) {
                                    ...
                                    api.sendMessage(ttt+"\n"+randomMessage, message.threadID, message.messageID);
                                }
                            });
                        }
...
function postToFacebook(api, content) {
  return new Promise((resolve, reject) => {
    api.postContent(content, (err, info) => {
      if (err) {
        reject(err);
      } else {
        resolve(info);
